...
import { FileDown, Calendar, TrendingUp, Car } from 'lucide-react';
...
const Reports = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const allEntries = await db.entries.filter(e => e.status === 'completed').toArray();
      const income = allEntries.reduce((acc, curr) => acc + (curr.total || 0), 0);
      ...
      const chartData = Object.keys(days).map(key => ({ name: key, total: days[key] }));
