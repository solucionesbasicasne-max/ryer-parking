...
import { Camera, ChevronRight, ChevronLeft, Save, UserPlus, Car, AlertTriangle, Search, PlusCircle, CreditCard, ShieldCheck } from 'lucide-react';
...
const CheckIn = ({ capacity, isCajaOpen }) => {
    ...
    if (formData.name.length > 1 && !formData.clientId) {
      const search = async () => {
        const results = await db.clients
          .filter(c => c.name.toLowerCase().includes(formData.name.toLowerCase()))
          .limit(5)
          .toArray();
  ...
  const selectClient = async (client) => {
    ...
    const now = Date.now();
    const activePlan = await db.entries
      .where('clientId').equals(client.id)
      .and(e => e.status === 'active' && e.nextPaymentDate && e.nextPaymentDate > now)
      .first();
  ...
  const handleSave = async () => {
    ...
    if (!finalClientId) {
       finalClientId = await db.clients.add({ name: formData.name, phone: formData.phone, email: formData.email });
    }

    const ticketNumber = (await db.entries.count() + 1).toString().padStart(6, '0');
    const nextPay = hasActivePlan ? null : getNextPaymentDate(Date.now(), formData.serviceType); // Skip payment date if already subscribed
    
    const rates = await db.rates.toArray();
    const rate = rates.find(r => r.id === formData.vehicleType.toLowerCase()) || rates[0];
    
    // Total calculation: 0 if valid subscription exists
    const finalPrice = hasActivePlan ? 0 : (formData.serviceType === 'Diario' ? rate.price6to6 : (formData.serviceType === 'Semanal' ? rate.price24h * 5 : rate.price24h * 15));

    const entryId = await db.entries.add({
      ticketNumber, clientId: finalClientId, plate: formData.plate.toUpperCase(),
      brand: formData.brand, model: formData.model, year: formData.year,
      vehicleType: formData.vehicleType, serviceType: formData.serviceType,
      entryTime: Date.now(), nextPaymentDate: nextPay, status: 'active',
      photos: formData.photos, total: finalPrice
    });

    const ticketDoc = await generateParkingTicket({
      id: entryId, ticketNumber, plate: formData.plate.toUpperCase(),
      brand: formData.brand, model: formData.model, entryTime: Date.now(),
      serviceType: formData.serviceType, nextPaymentDate: nextPay, total: finalPrice,
      isSubscriber: hasActivePlan
    });
  ...
  if (!isCajaOpen) {
    return (
      <div className="card text-center" style={{ padding: '4rem', color: 'var(--danger)' }}>
        ...
        <button className="btn btn-primary" style={{ marginTop: '2rem' }} onClick={() => window.location.hash = '#caja'}>Ir a Caja</button>
      </div>
    );
  }
  ...
  return (
    <div className="card" style={{ maxWidth: '800px', margin: '0 auto' }}>
      <div style={{ display: 'flex', gap: '1rem', marginBottom: '2rem' }}>
        {[1, 2, 3, 4].map(s => (
          <div key={s} style={{ flex: 1, height: '4px', background: step >= s ? 'var(--brand-primary)' : 'var(--bg-accent)', borderRadius: '2px' }} />
        ))}
      </div>
        ...
        <div>
          ...
          <div className="autocomplete-container input-group">
            ...
            <div style={{ position: 'relative' }}>
              <input type="text" value={formData.name} onChange={e => setFormData({...formData, name: e.target.value, clientId: null})} placeholder="Juan Perez..." />
          ...
          <div className="grid-2">
            <div className="input-group"><label className="input-label">Teléfono</label><input type="text" value={formData.phone} disabled={!!formData.clientId} onChange={e => setFormData({...formData, phone: e.target.value})} /></div>
            <div className="input-group"><label className="input-label">Email</label><input type="email" value={formData.email} disabled={!!formData.clientId} onChange={e => setFormData({...formData, email: e.target.value})} /></div>
          </div>
        </div>
      )}
      ...
      {step === 2 && (
        <div>
          ...
          <div className="grid-2">
            <div className="input-group"><label className="input-label">Placa</label><input type="text" value={formData.plate} onChange={e => setFormData({...formData, plate: e.target.value})} /></div>
      ...
      {step === 3 && (
        <div className="grid-2" style={{ gap: '1rem' }}>
          {['front', 'back', 'left', 'right'].map(key => (
            <div key={key} onClick={() => startCamera(key)} style={{ aspectRatio: '16/9', border: '2px dashed var(--border)', borderRadius: '8px', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', overflow: 'hidden' }}>
              {formData.photos[key] ? <img src={formData.photos[key]} style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : <span>{key.toUpperCase()}</span>}
            </div>
          ))}
        </div>
      )}
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '2rem' }}>
        ...
        <div style={{ marginLeft: 'auto' }}>
          {step < 3 ? <button className="btn btn-primary" onClick={() => setStep(step + 1)}>Siguiente</button> : <button className="btn btn-primary" style={{ background: hasActivePlan ? 'var(--brand-primary)' : 'var(--success)' }} onClick={handleSave}><Save size={20} /> {hasActivePlan ? 'Aceptar Socio' : 'Cobrar e Ingresar'}</button>}
        </div>
      </div>
       ...
       {cameraActive && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.9)', zIndex: 100, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '1rem' }}>
          ...
          <div style={{ display: 'flex', gap: '1rem' }}><button className="btn" onClick={stopCamera} style={{ background: 'var(--danger)' }}>Cerrar</button><button className="btn btn-primary" onClick={capturePhoto}>Capturar</button></div>
        </div>
      )}
    </div>
  );
